...
- **母稿改未遮的字**：投放稿里没自己改过的段跟着变成一样；投放稿自己改过的段不被母稿盖掉（按“段”三方合并，合并时本稿待处理批注同样跟着位置走）。已冻结的投放稿不再同步文字。
- **母稿确认新遮罩**：所有已派生的投放稿——包括已冻结的——在同一事务里把同一段遮掉；不管那段在投放稿里改没改过，原文一个字都不留。传播只向下（母稿 → 投放稿 → 投放稿的投放稿），绝不写回母稿。
...
npm test                                  # 68 项业务断言 + 34 项 HTTP 端到端断言
```
